...
type DAO interface {
	...
	ReadByUserID(userID string) (userAttribute []UserAttribute, err error)
	ReadByUserIDUnsafe(userID string) (userAttribute []UserAttribute)
}
...
func (d DAOImpl) CreateOrUpdate(userAttribute UserAttribute) (err error) {
	...
	if err == nil {
		if len(olds) == 0 {
			...
			old := olds[0]
			ids := idParams(old.UserID, old.AttrKey)
			err = d.Dynamo.UpdateTableEntry(
				allParams(userAttribute, old),
				ids,
				updateExpression(userAttribute, old),
				d.Name,
			)
			err = errors.Wrapf(err, "UserAttribute DAO.CreateOrUpdate(id = %v) couldn't UpdateTableEntry in table %s", ids, d.Name)
		}
	}
	return 
}
...
func (d DAOImpl)ReadByUserID(userID string) (out []UserAttribute, err error) {
...
func (d DAOImpl)ReadByUserIDUnsafe(userID string) (out []UserAttribute) {
...
func updateExpression(userAttribute UserAttribute, old UserAttribute) string {
	var updateParts []string
	
		
			
		if userAttribute.UserID != old.UserID { updateParts = append(updateParts, "user_id = :a0") }
		if userAttribute.AttrKey != old.AttrKey { updateParts = append(updateParts, "attr_key = :a1") }
		if userAttribute.AttrValue != old.AttrValue { updateParts = append(updateParts, "attr_value = :a2") }
		if userAttribute.Default != old.Default { updateParts = append(updateParts, "#default = :a3") }
