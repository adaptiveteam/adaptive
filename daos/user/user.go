...
type DAO interface {
	...
	Read(platformID common.PlatformID, id string) (user User, err error)
	ReadUnsafe(platformID common.PlatformID, id string) (user User)
	ReadOrEmpty(platformID common.PlatformID, id string) (user []User, err error)
	ReadOrEmptyUnsafe(platformID common.PlatformID, id string) (user []User)
...
func (d DAOImpl) CreateUnsafe(user User) {
	err2 := d.Create(user)
	core.ErrorHandler(err2, TableNameSuffixVar, fmt.Sprintf("Could not create platformID==%s, id==%s in %s\n", user.PlatformID, user.ID, TableName(d.ConnGen.TableNamePrefix)))
}


// Read reads User
func (d DAOImpl) Read(platformID common.PlatformID, id string) (out User, err error) {
	var outs []User
...
func (d DAOImpl) ReadUnsafe(platformID common.PlatformID, id string) User {
...
func (d DAOImpl) ReadOrEmpty(platformID common.PlatformID, id string) (out []User, err error) {
	var outOrEmpty User
...
func (d DAOImpl) ReadOrEmptyUnsafe(platformID common.PlatformID, id string) []User {
...
func (d DAOImpl) CreateOrUpdate(user User) (err error) {
	...
	var olds []User
	olds, err = d.ReadOrEmpty(user.PlatformID, user.ID)
...
func allParams(user User, old User) (params map[string]*dynamodb.AttributeValue) {
	params = map[string]*dynamodb.AttributeValue{}
	if user.PlatformID != old.PlatformID { params[":a0"] = common.DynS(string(user.PlatformID)) }
	if user.ID != old.ID { params[":a1"] = common.DynS(user.ID) }
	if user.DisplayName != old.DisplayName { params[":a2"] = common.DynS(user.DisplayName) }
	if user.FirstName != old.FirstName { params[":a3"] = common.DynS(user.FirstName) }
	if user.LastName != old.LastName { params[":a4"] = common.DynS(user.LastName) }
	if user.Timezone != old.Timezone { params[":a5"] = common.DynS(user.Timezone) }
	if user.IsAdaptiveBot != old.IsAdaptiveBot { params[":a6"] = common.DynBOOL(user.IsAdaptiveBot) }
	if user.TimezoneOffset != old.TimezoneOffset { params[":a7"] = common.DynN(user.TimezoneOffset) }
	if user.AdaptiveScheduledTime != old.AdaptiveScheduledTime { params[":a8"] = common.DynS(user.AdaptiveScheduledTime) }
	if user.AdaptiveScheduledTimeInUTC != old.AdaptiveScheduledTimeInUTC { params[":a9"] = common.DynS(user.AdaptiveScheduledTimeInUTC) }
...
func updateExpression(user User, old User) (expr string, params map[string]*dynamodb.AttributeValue, namesPtr *map[string]*string) {
	...
	if user.PlatformID != old.PlatformID { updateParts = append(updateParts, "platform_id = :a0"); params[":a0"] = common.DynS(string(user.PlatformID));  }
	if user.ID != old.ID { updateParts = append(updateParts, "id = :a1"); params[":a1"] = common.DynS(user.ID);  }
	if user.DisplayName != old.DisplayName { updateParts = append(updateParts, "display_name = :a2"); params[":a2"] = common.DynS(user.DisplayName);  }
	if user.FirstName != old.FirstName { updateParts = append(updateParts, "first_name = :a3"); params[":a3"] = common.DynS(user.FirstName);  }
	if user.LastName != old.LastName { updateParts = append(updateParts, "last_name = :a4"); params[":a4"] = common.DynS(user.LastName);  }
	if user.Timezone != old.Timezone { updateParts = append(updateParts, "#timezone = :a5"); params[":a5"] = common.DynS(user.Timezone); fldName := "timezone"; names["#timezone"] = &fldName }
	if user.IsAdaptiveBot != old.IsAdaptiveBot { updateParts = append(updateParts, "is_adaptive_bot = :a6"); params[":a6"] = common.DynBOOL(user.IsAdaptiveBot);  }
	if user.TimezoneOffset != old.TimezoneOffset { updateParts = append(updateParts, "timezone_offset = :a7"); params[":a7"] = common.DynN(user.TimezoneOffset);  }
	if user.AdaptiveScheduledTime != old.AdaptiveScheduledTime { updateParts = append(updateParts, "adaptive_scheduled_time = :a8"); params[":a8"] = common.DynS(user.AdaptiveScheduledTime);  }
	if user.AdaptiveScheduledTimeInUTC != old.AdaptiveScheduledTimeInUTC { updateParts = append(updateParts, "adaptive_scheduled_time_in_utc = :a9"); params[":a9"] = common.DynS(user.AdaptiveScheduledTimeInUTC);  }
